...
<script>
    ...
    function store(modo, id) {
        ...
        if (cnombre.length == 0) {
            toastr.error('Ingrese un noombre de Flete', 'Mensaje del Sistema');
            return;
        }
        ...
        if (modo == 'N') {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    axios.post('/admin/flete/store', data)
                        .then(function(respuesta) {
                            $('#modal-mantenimiento').modal('hide');
                            toastr.success('Flete registrado correctamente', 'Mensaje del Sistema');
                            buscar();
                        }).catch(function(error) {
            ...
            Swal.fire({
                ...
                text: "Se modificarpa con los nuevos datos",
                icon: 'question',
                showCancelButton: true,
                ...
                if (result.isConfirmed) {
                    axios.post(ruta, data)
                        .then(function() {
                            $('#modal-mantenimiento').modal('hide');
                            buscar();
                            toastr.success('Flete actualizado correctamente', 'Mensaje del Sistema');
                        }).catch(function(error) {
                            if (error.hasOwnProperty('response')) {
                                if (error.response.status === 422) {
