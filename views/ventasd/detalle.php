                ...
                <?php if (!empty($item['activo'])): ?>
                    <?php if ($item['activo'] == 'A') { ?>
                        <tr onkeyup="verificarValores(this); actualizarProducto(this,<?php echo $indice ?>,event);" onblur="actualizarProducto(this,<?php echo $indice ?>,event);">
                            ...
                            <td class="text-center cantidad" onclick="funcionEnterCant(this,<?php echo $indice ?>)" contenteditable="false" name="cantidad"><input type="text" class="inputright" onkeyup="abrirpreregistro(event)" onkeypress="return isNumber(event);" value="<?php echo number_format($item['cantidad'], 3, '.', '') ?>"></td>
                            <td class="precio text-center" id="precio" contenteditable="false" name="precio"><input onkeypress="return isNumber(event);" type="text" class="inputright" onkeyup="abrirpreregistro(event)" readonly value="<?php echo number_format($item['precio'], 2, '.', '') ?>"></td>
                            <td class="preciosgv text-center"></td>
                            <td class="descuento text-end"><input onkeypress="return isNumber(event);" type="text" onclick="this.select();" class="inputright" value=""></td>
                            <td class="text-center" class="total"><input onclick="ubicacionfocus='subtotal'; $(this).select();" type="text" class="inputright" onkeypress="ubicacionfocus='subtotal'; return isNumber(event);" value="<?php echo number_format(round($item['cantidad'] * $item['precio'], 2), 2, '.', '') ?>"></td>
                            <?php $i++; ?>
                        </tr>
                    <?php } ?>
                <?php endif; ?>
            ...
            <div class="col-6 align-items-start">
                ...
                <div class="input-group">
                    ...
                    <!-- <?php if ($_SESSION['tipousuario'] == 'A') : ?>
                        <button class="btn btn-warning btn-sm" style="display:none" onclick="verutilidad();">Ver Utilidad</button>
                    <?php endif; ?> -->
                </div>
            </div>
...
<script>
    ...
    function actualizarProducto(o, i, event) {
        ...
        var _tr = $(o);
        const data = new FormData();
        cantidad = _tr.find("td").eq(4).find("input").val();
        if (cantidad.length == 0) {
            $("#grabar").attr("disabled", true);
            return;
        }
        cantidad = cantidad.replace(",", "");
        _tr.find("td").eq(4).find("input").val(cantidad)
        precio = _tr.find("td").eq(5).find("input").val();
        if (precio.length == 0) {
            $("#grabar").attr("disabled", true);
            return;
        }
        precio = precio.replace(",", "");
        _tr.find("td").eq(5).find("input").val(precio)
        ...
        axios.post('/vtas/EditarUno', data)
            .then(function(respuesta) {
                ...
                if (trim(error) == 'Network Error') {
                    const datanew = new FormData();
                    datanew.append("txtdescri", _tr.find("td").eq(2).html());
                    datanew.append("txtprecio", _tr.find("td").eq(5).find("input").val());
                    datanew.append("txtcantidad", _tr.find("td").eq(4).find("input").val());
                    datanew.append("cmbmoneda", $("#cmbmoneda").val());
                    datanew.append("indice", i);
                    axios.post('/vtas/EditarUno', datanew)
                        .then(function(respuesta) {
                            calcularIGV();
                        });
                }
    ...
    function verificarValores(o) {
        ...
        <?php endif; ?>
        if (cmbmoneda == 'D') {
            preciomenor = (Number(precios[0])) / Number("<?php echo $_SESSION["gene_dola"] ?>");
        } else {
            preciomenor = Number(precios[0]);
        }
    ...
    function abrirpreregistro(e) {
        if (e.keyCode == 27) {
            validarvaloresporgrupo()
            if (error == "0") {
                $("#mddatosapagar").modal("show");
            } else {
                toastr.error("No se puede grabar porque hay costos / precios debajo de lo establecido.", 'Mensaje del Sistema')
            }
        }
    }
